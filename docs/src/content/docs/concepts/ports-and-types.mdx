...
One constraint. Fields that decide canvas padding at build time — blur sigma and
friends — need a **static upper bound**: a literal, or a `$param` whose
declaration has `max`. Wiring those from a `@node` port is a build error, because
padding must be known before anything is evaluated. See
