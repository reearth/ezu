...
| `--pad` | margin for geometry painted wider than its own extent, e.g. a thick stroke (default 16). Filter reach is sized by the renderer from the graph, so this is only about paint width — see [padding](/ezu/concepts/padding-and-neighbours/) |
| `--keep-hidden` | keep `visibility: none` layers, gated off behind a `switch` instead of dropped |
| `--migrate` | convert the style's legacy forms first — `{stops}` function objects, `{name}` label tokens, legacy filters, non-standard `hsl()` colours — as MapLibre's own `migrate` does. Pass it for a style written before expressions; without it a legacy form is left as it is and named in a warning |
