...
if err := r.BindSource(ctx, "basemap", mvt, ezu.Bind{}); err != nil {
    return err
}
...
pass the text as `ezu.Bind{Index: …}` alongside the atlas image.
...
```go
func renderTile(ctx context.Context, r *ezu.Renderer, z uint8, x, y uint32) ([]byte, error) {
    sources, err := r.Sources(ctx)
    if err != nil {
        return nil, err
    }
    ...
    for _, s := range sources {
        ...
        tile, err := r.SourceTile(ctx, s.Name, z, int32(x), int32(y))
        if err != nil {
            return nil, err
        }
        body, err := fetch(tileURL(s.URL, tile))
        if err != nil {
            return nil, err
        }
        if err := r.BindSource(ctx, s.Name, body, ezu.Bind{SourceZoom: int(tile.Z)}); err != nil {
            return nil, err
        }
        ...
        for _, o := range offsets {
            nt, err := r.SourceTile(ctx, s.Name, z, int32(x)+int32(o.DX), int32(y)+int32(o.DY))
            if err != nil {
                return nil, err
            }
            body, err := fetch(tileURL(s.URL, nt))
            if err != nil {
                continue // off the top or bottom of the world: that edge clamps
            }
            if err := r.BindSource(ctx, s.Name, body, ezu.Bind{
                DX: o.DX, DY: o.DY, SourceZoom: int(nt.Z),
            }); err != nil {
                return nil, err
            }
        }
    ...
    return r.RenderTile(ctx, z, x, y, ezu.Render{})
}
```
...
- **Missing glyphs are not an error.** Text whose glyphs were never bound is
  dropped with a warning, and warnings arrive through `InitLog` / `DrainLogs`
  rather than as a returned error. `NeededCodepoints` is the precise form of
  `NeededGlyphRanges` for a host that can assemble its own PBF; on CJK labels that
  is the difference between a few thousand glyphs and tens of megabytes.
...
err = pool.Do(ctx, func(r *ezu.Renderer) error {
    out, err := renderTile(ctx, r, z, x, y)
    if err != nil {
        return err
    }
    return write(out)
...
- **A custom `CompilationCache` panics.** The option's parameter is an interface,
  but wazero type-asserts it to its own concrete type without checking, so the
  only value you may pass is the return of `wazero.NewCompilationCacheWithDir`.
