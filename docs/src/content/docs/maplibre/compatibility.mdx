...
a real font per entry for outline rendering. A `{token}` field is an expression
once the style is migrated (`--migrate`, see below).
...
| zoom / data functions (`interpolate`, `step`, any expression) | emitted raw onto the target node's `*-expr` field, evaluated per tile |
| legacy forms — `{stops}` function objects, `{token}` label strings, non-standard `hsl()` colours | converted only with `--migrate` (`ConvertOptions::migrate`), by `maplibre_expr::migrate` — MapLibre's own `migrate`, each value converted with its property spec. Off by default: a legacy form is passed through untouched and named in a warning; a function object is then rejected by ezu-paint, a token string renders literally |
