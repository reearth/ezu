...
`npm run compare` additionally needs the Playwright reference renderer
...
- Add a line under `## Unreleased` in
  [`CHANGELOG.md`](https://github.com/reearth/ezu/blob/main/CHANGELOG.md) if the
  change is one a user would notice — a new capability, a breaking change, a
  fix to something they could have hit, or a render that moves. Internal
  refactors need no entry. That section becomes the next release's notes
  verbatim, so write it for someone deciding whether to upgrade.
